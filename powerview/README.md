...
Find-LocalAdminAccess -domain domain.local
...
Get-NetSession -ComputerName dc | select CName, UserName
...
# specific group SID
get-domaingroup -identity "domain admins" -domain domain.local -properties cn,objectsid
...
Get-DomainObjectAcl -SearchBase 'CN=AdminSDHolder,CN=System,DC=domain,DC=local' | %{ $_.SecurityIdentifier } | Convert-SidToName
...
Test-AdminAccess -ComputerName dc.domain.local
...
# list GPOs linked to domain and OUs with ADSI LDAP
(([adsi]'LDAP://DC=domain,DC=local'),(([adsisearcher]'(objectcategory=organizationalunit)')).findall()).Path | %{if(([ADSI]"$_").gPlink){Write-Host "[+] OU Path:"([ADSI]"$_").Path;$a=((([ADSI]"$_").gplink) -replace "[[;]" -split "]");for($i=0;$i -lt $a.length;$i++){if($a[$i]){Write-Host "Policy Path[$i]:"([ADSI]($a[$i]).Substring(0,$a[$i].length-1)).Path;Write-Host "Policy Name[$i]:"([ADSI]($a[$i]).Substring(0,$a[$i].length-1)).DisplayName} };Write-Output "`n" }}

# look for AppLocker GPO
Get-DomainGPO -Domain domain.local | ? { $_.DisplayName -like "*AppLocker*" } | select displayname, gpcfilesyspath
Get-DomainGPO -Domain domain.local | ? { $_.DisplayName -like "*ocker*" } | select displayname, gpcfilesyspath
...
Get-DomainObjectAcl -SearchBase "LDAP://OU=Workstations,DC=domain,DC=local" -ResolveGUIDs | ? { $_.ObjectAceType -eq "ms-Mcs-AdmPwd" -and $_.ActiveDirectoryRights -like "*ReadProperty*" } | select ObjectDN, SecurityIdentifier
...
Get-DomainObjectAcl -Identity "CN=Policies,CN=System,DC=domain,DC=local" -ResolveGUIDs | ? { $_.ObjectAceType -eq "Group-Policy-Container" -and $_.ActiveDirectoryRights -contains "CreateChild" } | % { ConvertFrom-SID $_.SecurityIdentifier }
...
Get-DomainGPO -Identity "CN={AD7EE1ED-CDC8-4994-AE0F-50BA8B264829},CN=Policies,CN=System,DC=domain,DC=local" | select displayName, gpcFileSysPath
...
# list WMI filters
Get-DomainObject -SearchBase "CN=SOM,CN=WmiPolicy,CN=System,DC=domain,DC=local" -LDAPFilter "(objectclass=msWMI-Som)" -Properties name,mswmi-name,mswmi-parm2 | fl

# check filter name GUID against gpcwqlfilter to find the GPO they're linked to
Get-DomainObject -SearchBase "CN=Policies,CN=System,DC=domain,DC=local" -LDAPFilter "(objectclass=groupPolicyContainer)" -Properties displayname,gpcwqlfilter | fl
...
# get GPO of an OU
Get-DomainGPO -SearchBase "LDAP://cn={16927C64-CF83-4962-B0CF-6F90710F19C8},cn=policies,cn=system,DC=domain,DC=local" -properties displayname,name,gpcfilesyspath | fl
...
- [Unhide a Hidden GPO](https://pentestlibrary.blogspot.com/2019/07/unhide-hidden-domain-gpo.html)
